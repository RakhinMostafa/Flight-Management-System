    ...
    <div class="container-fluid text-center">
        <div class="row content">
            ...
            <div class="col-sm-8 text-left">
                ...
                $sql = "SELECT FL.number AS FLnumber, company, type, departure, d_time, arrival, flightTime, C.name AS classname, capacity, price, COUNT(*)
                FROM flight FL,  class C, airplane AP , airport A
                WHERE (FL.number = C.number) AND (FL.airplane_id = AP.ID) AND C.name = '$class' AND FL.flightTime = '$depart' AND            
               ...
               OR (((departure LIKE '%$from%') AND (arrival LIKE '%$to%'))) )
                ...
                $sql2 = "SELECT FL.number AS FLnumber, company, type, departure, d_time, arrival, flightTime, C.name AS classname, capacity, price, COUNT(*)
                FROM flight FL,  class C, airplane AP , airport A
                WHERE (FL.number = C.number) AND (FL.airplane_id = AP.ID) AND C.name = '$class' AND FL.flightTime = '$return' AND            
               ((((city LIKE '%$to%') AND (code = departure)) OR  ((city LIKE '%$from%') AND (code = arrival)))
               OR (((departure LIKE '%$to%') AND (arrival LIKE '%$from%'))) )
                GROUP BY FL.number
                HAVING COUNT(*)>1
                ORDER BY C.price";
